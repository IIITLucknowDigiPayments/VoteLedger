...
// ShadowVote contract ABI - Updated for Solidity version
const contractABI = [
  // Events
  "event PollCreated(uint64 indexed pollId, address indexed creator, string question, uint256 optionsCount, uint256 endTime)",
  "event VoteCast(uint64 indexed pollId, address indexed voter, uint256 timestamp)",
  "event PollClosed(uint64 indexed pollId, address indexed closer)",
  "event PollExtended(uint64 indexed pollId, uint256 newEndTime)",
  
  // Core Functions
  "function createPoll(string calldata question, string[] calldata options, uint256 duration) external returns (uint64)",
  ...
  // View Functions
  "function getPoll(uint64 pollId) external view returns (uint64 id, string memory question, string[] memory options, uint64[] memory counts, address creator, bool isActive, uint256 createdAt, uint256 endTime)",
  "function getPolls(uint256 offset, uint256 limit) external view returns (uint64[] memory ids, string[] memory questions, address[] memory creators, bool[] memory activeStatus, uint256[] memory voteCounts)",
  "function getActivePolls() external view returns (uint64[] memory activePollIds)",
  "function hasVoted(uint64 pollId, address voter) external view returns (bool)",
  "function getTotalPolls() external view returns (uint256)",
  "function getPollResults(uint64 pollId) external view returns (string[] memory options, uint64[] memory counts, uint256 totalVotes)",
  "function isPollActive(uint64 pollId) external view returns (bool)",
  
  // Admin Functions
  "function owner() external view returns (address)",
  "function nextPollId() external view returns (uint64)",
  "function transferOwnership(address newOwner) external"
] as const;

// Contract address from deployment
const CONTRACT_ADDRESS = "0xb08Ea2E35167a66009356A717B38722DC65346C1"
  ...
  async createPoll(question: string, options: string[]): Promise<string> {
    try {
      if (this.isContractAvailable && this.contract) {
        ...
        // Poll duration: 7 days in seconds (0 for unlimited)
        const duration = 7 * 24 * 60 * 60;
        const tx = await contractWithSigner.createPoll(question, options, duration);
  ...
  async getAllPolls(): Promise<Poll[]> {
    try {
      if (this.isContractAvailable && this.contract) {
        ...
        const total = await this.contract.getTotalPolls();
        ...
        // Fetch all polls (with pagination if needed)
        const limit = 100; // Fetch in batches of 100
        const allPolls: Poll[] = [];
        
        for (let offset = 0; offset < totalNum; offset += limit) {
          const pollsData = await this.contract.getPolls(offset, limit);
          
          for (let i = 0; i < pollsData.ids.length; i++) {
            const pollId = pollsData.ids[i].toString();
